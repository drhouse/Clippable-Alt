...
function grabArticle() {
    ...
    for (var j = 0; j < allParagraphs.length; j++) {
        ...
        if (typeof parentNode.readability == 'undefined') {
            ...
            else if (parentNode.className.match(
    ...
    for (nodeIndex = 0;
        ...
        if (typeof node.readability != 'undefined' && (topDiv == null || node.readability.contentScore > topDiv
    ...
    if (topDiv == null) {
        ...
        topDiv.innerHTML =
            'Sorry, clippable was unable to parse this page for content. If you feel like it should have been able to, please <a href="http://brettterpstra.com/contact">let us know.</a>';
    }
...
function killDivs(e) {
    ...
    for (var i = curDivLength - 1; i >= 0; i--) {
        var p = divsList[i].getElementsByTagName("p")
            .length;
        var img = divsList[i].getElementsByTagName("img")
            .length;
        var li = divsList[i].getElementsByTagName("li")
            .length;
        var a = divsList[i].getElementsByTagName("a")
            .length;
        var embed = divsList[i].getElementsByTagName("embed")
            .length;
        var object = divsList[i].getElementsByTagName("object")
            .length;
        var pre = divsList[i].getElementsByTagName("pre")
            .length;
        var code = divsList[i].getElementsByTagName("code")
            .length;
        ...
        if (getCharCount(divsList[i]) < 10) {
            ...
            if ((img > p || li > p || a > p || p == 0 || divId.match("comment") != null || divClass.match("comment") !=
                    null || divId.match("share") != null || divClass.match("share") != null) && (pre == 0 && code ==
                    0 && embed == 0 && object == 0 && sphereit == 0)) {
                if (!p == 0 && img == 1) {
...
function clean(e, tags, minWords) {
    ...
    if (tags == "table") {
        ...
        for (y = 0; y < targetList.length; y++) {
            ...
            cells = targetList[y].getElementsByTagName('td')
                .length;
            if (cells < minWords) {
